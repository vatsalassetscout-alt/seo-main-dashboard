...
export function RankTracker({ theme }: { theme: 'light' | 'dark' }) {
  ...
  // 2. Load Trackers from local storage OR standard sheets API (Auto-fallback)
  ...
  const handleSave = async (updatedTrackers: Tracker[]) => {
    ...
    try {
      ...
      console.warn(`Google Sheet background sync failed: ${errMsg}`);
    }
  };
  ...
  const domainGroups = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([domain, kws]) => {
      ...
      if (positiveRanks.length > 0) {
        ...
        const hasChecking = kws.some((k) => k.pos === 0);
        if (hasChecking) {
          best = null;
        } else {
          const hasNotFound = kws.some((k) => k.pos === -1);
          if (hasNotFound) {
            best = -1;
          } else {
            best = null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full">
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800/80 p-5 rounded-2xl shadow-xs transition-all">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Enter your premium <a href="https://serpapi.com" target="_blank" rel="noreferrer" className="text-indigo-500 underline font-semibold">SerpAPI</a> key to execute real-time search engine audits. If empty, the tracker automatically falls back to high-performance organic search parsers.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto md:min-w-[320px]">
              ...
              <button
                ...
                className="p-2 px-5 rounded-xl font-extrabold text-xs bg-indigo-600 hover:bg-indigo-700 text-white cursor-pointer active:scale-95 transition-all h-[38px] shadow-md shadow-indigo-600/10 shrink-0"
              >
                Save Key
              </button>
            </div>
